...
static void menu_draw_header_callback(GContext* ctx, const Layer *cell_layer, uint16_t section_index, void *data) {
  ...
  /*switch (section_index) {
    case 0:
      ...
      menu_cell_basic_header_draw(ctx, cell_layer, "Tasks");
      /*break;
...
static void menu_draw_header_callback1(GContext* ctx, const Layer *cell_layer, uint16_t section_index, void *data) {
  // Determine which section we're working with
  /*switch (section_index) {
    case 0:
      // Draw title text in the section header*/
      menu_cell_basic_header_draw(ctx, cell_layer, "Subtasks");
      /*break;
    ...
    case 0:
      ...
      switch (cell_index->row) {
        case 0:
          ...
          menu_cell_basic_draw(ctx, cell_layer, "Task Name", "Time Left", NULL);
          break;

        case 1:
          // This is a basic menu icon with a cycling icon
          menu_cell_basic_draw(ctx, cell_layer, "Something Else", "Time Left", NULL);
          break;
...
static void menu_draw_row_callback1(GContext* ctx, const Layer *cell_layer, MenuIndex *cell_index, void *data) {
  // Determine which section we're going to draw in
  /*switch (cell_index->section) {
    case 0:
      // Use the row to specify which item we'll draw
      switch (cell_index->row) {
        case 0:*/
          // This is a basic menu item with a title and subtitle
          menu_cell_basic_draw(ctx, cell_layer, "Subtask Name", "Time Left", NULL);
          /*break;
...
void window_unload1(Window *window) {
  ...
  menu_layer_destroy(menu_layer1);
...
void menu_select_callback1(MenuLayer *menu_layer, MenuIndex *cell_index, void *data) {
  ...
  //make new window with subtasks of current task
...
void window_load1(Window *window) {
  ...
  menu_layer1 = menu_layer_create(bounds);

  // Set all the callbacks for the menu layer
  menu_layer_set_callbacks(menu_layer1, NULL, (MenuLayerCallbacks){
    .get_num_sections = menu_get_num_sections_callback,
    .get_num_rows = menu_get_num_rows_callback,
    .get_header_height = menu_get_header_height_callback,
    .draw_header = menu_draw_header_callback1,
    .draw_row = menu_draw_row_callback1,
    .select_click = menu_select_callback1,
  });

  // Bind the menu layer's click config provider to the window for interactivity
  menu_layer_set_click_config_onto_window(menu_layer1, window1);

  // Add it to the window for display
  layer_add_child(window_layer, menu_layer_get_layer(menu_layer1));
}
